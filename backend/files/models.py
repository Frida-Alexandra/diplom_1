...
def user_directory_path(instance, filename):
    file_name, file_path = get_unique_file_name_and_file_path(instance.user,
                                                              filename)
...
class File(models.Model):
    ...
    file_name = models.CharField(
        blank=True, default="", max_length=255, verbose_name="Имя файла"
    )
    ...
    def save(self, *args, **kwargs):
        if self.file_name:
            ...
            original_file_name = "_".join(os.path.basename(self.file.name)
                                          .split())
            if self_file_name != original_file_name:
                ...
                if self.id:
                    ...
                    file_name, file_path = get_unique_file_name_and_file_path(
                        self.user, new_file_name
                    )
                    self.file_name = file_name
                    self.file_path = file_path
                    self.file.name = file_path
                    new_full_file_path = os.path.join(
                        settings.MEDIA_ROOT, self.file_path
                    )
