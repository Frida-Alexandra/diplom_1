...
from cloud_service.cloud.models import FileModel, User
from cloud_service.cloud.storage_file_name import generate_storage_file_name
from cloud_service.cloud.download_id import generate_download_id
from cloud_service.cloud.validators import patchValidator
...
class FileSerializer(serializers.ModelSerializer):
    ...
    def patch(self, **kwargs):
        ...
        if kwargs["user"].is_staff:
            ...
            file = (
                FileModel.objects.filter(user_id=kwargs["user"].id)
                .filter(id=validated_data["id"])
                .first()
            )
